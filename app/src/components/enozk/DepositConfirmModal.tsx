...
import {
  ...
  evmSelector4,
  NOZK_VAULT_DEPOSIT_SELECTOR_HEX,
  parseNozkVaultDepositCalldataArgs,
} from '../../crypto/nozkDeposit'
...
export function DepositConfirmModal({ open, onClose, onToast }: Props) {
  ...
  const handleContinue = async () => {
    ...
    try {
      ...
      try {
        const built = await buildNozkVaultDepositCalldata(seed, tokenIndex)
        data = built.data
        builtDepositId = built.depositId
      } catch (err) {
        console.error('NozkVault deposit calldata', err)
        onToast('Could not build blind deposit payload', 'error')
        return
      }

      if (!data || data === '0x' || data.length < 10) {
        console.error('[NozkVault deposit] missing calldata — would be plain ETH transfer', {
          data,
        })
      ...
      const sendParams = {
        ...
        data,
        value: NOZK_VAULT_DEPOSIT_VALUE_WEI_HEX,
      }
      let depositFnArgs: ReturnType<typeof parseNozkVaultDepositCalldataArgs>
      try {
        ...
        if (isNozkVaultDebugEnabled()) {
          console.warn('[NozkVault deposit debug] parse calldata failed', e)
        }
      }
      ...
      const selectorMatchesDepositAbi =
        calldataSelector === NOZK_VAULT_DEPOSIT_SELECTOR_HEX.toLowerCase()
      ...
      try {
        onChainDenominationWeiHex = await chainRpcCall<string>('eth_call', [
          { to: NOZK_VAULT_ADDRESS, data: evmSelector4('DENOMINATION()') },
          'latest',
        ])
      } catch (denErr) {
        if (isNozkVaultDebugEnabled()) {
          console.warn('[NozkVault deposit debug] DENOMINATION() eth_call failed', denErr)
        }
      }
      try {
      ...
      if (isNozkVaultDebugEnabled()) {
        console.log('[NozkVault deposit debug] before eth_sendTransaction', {
          tokenIndex,
          chainId,
          vaultTo: NOZK_VAULT_ADDRESS,
          txValueWeiHex: sendParams.value,
          txValueWeiDecimal: txValueWei.toString(),
          calldataSelector,
          expectedDepositSelector: NOZK_VAULT_DEPOSIT_SELECTOR_HEX,
      ...
      try {
        ...
        if (isNozkVaultDebugEnabled()) {
          console.log(
            '[NozkVault deposit debug] eth_call (same as tx): ok — no revert on this RPC'
          )
        }
      } catch (simErr) {
        if (isNozkVaultDebugEnabled()) {
          console.warn(
            '[NozkVault deposit debug] eth_call (same as tx): revert / error (useful detail if the node returns it)',
            simErr
          )
        }
      }
