...
```
├── nozk_library.py          # Python cryptographic library (source of truth)
├── nozk-library.ts          # TypeScript port (byte-for-byte parity)
...
│
├── nozk_library_test.py     # Python unit tests (20 cases)
...
The Python library (`nozk_library.py`) is the source of truth. The TypeScript library (`nozk-library.ts` + `bn254-crypto.ts`) is a byte-for-byte port. Both expose the same logical operations with language-appropriate conventions.
...
```python
# Python
from nozk_library import generate_mint_keypair
...
import { generateMintKeypair } from './nozk-library.js';
await initBN254();
...
Wallet state is cached in `.nozk_wallet.json` but is fully recoverable from the seed via `scan`.
...
| `CONTRACT_ADDRESS` | both | Deployed NozkVault address |
...
```bash
uv run pytest nozk_library_test.py -v    # 20 tests
```
...
```bash
uv run nozk_tip_test.py    # Python
