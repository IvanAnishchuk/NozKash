"""
Nozk Protocol: Key & Configuration Generator

Generates all secrets and configuration needed to run the Nozk client,
mint, and flow scripts. Writes a complete .env file with:
    ...
    • CONTRACT_ADDRESS        NozkVault contract address
...
"""
    ...
    if contract_address:
        chain_section += f"""
# ── Contract ──────────────────────────────────────────────────────────────────
# NozkVault contract address on Sepolia.
...
# For offline testing, run:  ./nozk_flow.sh --to 0xAnyAddress --mock
#
...
app = typer.Typer(
    name="generate-keys",
    help="Generate Nozk Protocol keys and configuration.",
