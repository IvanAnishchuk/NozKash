#!/usr/bin/env bash
# =============================================================================
# nozk_flow.sh — Nozk Protocol: Full Lifecycle Runner
#
# Drives the complete Nozk eCash flow from a single script:
...
#   ./nozk_flow.sh --to 0xYourAddress
#
#   # Dry-run: generate all payloads without broadcasting
#   ./nozk_flow.sh --to 0xYourAddress --dry-run
#
#   # MOCK: full offline test — no chain, no gas, no RPC
#   ./nozk_flow.sh --to 0xYourAddress --mock
#
#   # Resume from scan (deposit already done)
#   ./nozk_flow.sh --to 0xYourAddress --skip-deposit
#
#   # Use a relayer for gas-free redemption
#   ./nozk_flow.sh --to 0xYourAddress --relayer http://localhost:8000
# =============================================================================
...
# ── Helpers ───────────────────────────────────────────────────────────────────
log()      { echo -e "${CYAN}${BOLD}[nozk]${RESET}  $*"; }
...
if $DRY_RUN; then
    ...
    echo -e "  ${GREEN}${BOLD}🎉  NOZK FLOW COMPLETE${RESET}"
