...
import { initBN254, getG2Generator, CURVE_ORDER } from './bn254-crypto.js';
...
async function main() {
    ...
    const proof = await gl.generateRedemptionProof(gl.getSpendPriv(secrets), destination);

    console.log(`    Destination      : ${destination}`);
    console.log(`    msg_hash         : ${Buffer.from(proof.msgHash).toString('hex')}`);
